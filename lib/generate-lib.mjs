...
const createDirIfNotExists = async dirPath => {
  const subpaths = dirPath
    ...
    .reduce((prev, cur, index) => {
      ...
      prev.push(path.join(prevPath, cur));
      return prev;
    }, []);
...
export const convertSingleImage = async (relativeSource, options = {}) => {
  const sourceFile = path.join("./image-source-files", relativeSource);
  ...
  const outputDir = path.join("./public/images/generated", outputSubPath);
...
const generateImageReexporter = async (generatedFilesRelative, relativePath) => {
  ...
  const outputDir = path.join("./data/generated/capsules", ...parentDirs);
  await createDirIfNotExists(outputDir);
...
export const convertImages = async (relativeSource, options = {}) => {
  const sourceDir = path.join("./image-source-files", relativeSource);
  const outputDir = path.join("./public/images/generated", relativeSource);
